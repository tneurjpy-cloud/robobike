...
static void wifi_init_softap(void)
{
    ...
    // 1〜13(JAPAN ONLY) 1-11(World wide)
    uint8_t channel = ((((uint16_t)mac[4] << 8 | (uint16_t)mac[5])) % 11) + 1; // 1 - 11
...
void webserver_start()
{
    ...
    { //////////////////////////////////////// register uri_handlers
        extern const httpd_uri_t *uri_handlers[];
        extern const size_t uri_handlers_count;
        for (int i = 0; i < uri_handlers_count; i++)
        {
            httpd_register_uri_handler(http_server, uri_handlers[i]);
        }
