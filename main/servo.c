...
static void push_data(Tvector6d *new_data)
{
    Tlogvector *p;
...
const char *get_unread_data()
{
    ...
    while (index_r != index_w)
    {
        ...
        // データ範囲= +-100. +-90. +-90. +-5. +-90. +-90.
        int len = snprintf(item_buf, sizeof(item_buf),
                           "a,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n",
                           p->drv, p->str, p->std, p->acc.gx, p->acc.gy, p->acc.gz);

        // バッファ溢れチェック（既存の文字列長 + 新規追加分 + 終端ヌル文字）
